...
template<class T>
bool Edge<T>::operator<(Edge<T> *second) {
    if(weight < second->weight) return true;
    else return false;
}
...
protected:
    ...
    bool addReservoir(const Reservoir &in);
};
...
template <class T>
Vertex<T>::Vertex(T in): station(in) {}
/*
